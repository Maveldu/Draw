 ...
 * @param x pointing the location to memorize the x-coordinate of a click
 * @param y pointing the location to memorize the y-coordinate of a click
 * @return 0 if the application should quit, 1 by default, 2 if c pressed, 3 if f pressed, 4 if l pressed, 5 if r pressed, 6 if coords selected
 */
int checkEvents(unsigned long* x,unsigned long* y) {
  SDL_Event e;
  ...
  return 1;
} // checkEvents(unsigned long*,unsigned long*)
...
/** Fill the persistent screen
 * @param a the color's alpha  component
 * @param r the color's red component
 * @param g the color's green component
 * @param b the color's blue component
 */
void displayClear(int a, int r, int g, int b){
	if (SDL_SetRenderTarget(sdlRenderer, sdlScreenTexture) != 0)
	    printf("SDL_SetRenderTarget failed : %s\n", SDL_GetError());
	if (SDL_SetRenderDrawColor(sdlRenderer, r, g, b, a) != 0) {
		printf("SDL_SetRenderDrawColor failed : %s\n",SDL_GetError());
